...
class RoomStatusClass extends React.Component<PropsType, StateType> {
    ...
    render() {
        const { id, name, propertyName } = this.props;
        const { room_service, do_not_disturb } = this.state;
        ...
        var color: string = propertyName === 'room_service' ? '#37BA37' : Colors.red;
        ...
        return (
            <Panel active={isActive} blocks={2} onPress={() => this.changeStatus(1 - isActive)}>
                ...
                <View style={I18n.l2r() ? styles.texts : styles.texts_r2l}>
                    ...
                    <Text style={[styles.info, isActive ? {color: Colors.red} : {}]}>{isActive ? I18n.t(propertyName === 'room_service' ? "Housekeeping on the way" : "No one will disturb you") : " "}</Text>
                </View>
            </Panel>
        );
    }
